namespace :instant_win do
  ...
  def createMaxibonWins
    ...
    InstantWinPrize.create(:id => MAXIBON_DAILY_ACCESS, :title => "Un ingresso gratuito giornaliero all'Aquafan", :description => "Hai vinto un ingresso gratuito giornaliero all' Aquafan di Riccione.", :contest_periodicity_id => contest_periodicity_1.id)
    InstantWinPrize.create(:id => MAXIBON_PARTY_GUETTA, :title => "Un pacchetto di 5 biglietti per il Maxiparty con David Guetta", :description => "Pacchetto di 5 biglietti per l'ingresso gratuito all'evento con David Guetta", :contest_periodicity_id => contest_periodicity_2.id)
    InstantWinPrize.create(:id => MAXIBON_PARTY_NIGHT, :title => "Un pacchetto di 5 biglietti per un evento serale Aquafan", :description => "Pacchetto di 5 biglietti per l'ingresso gratuito a un evento serale di Aquafan, verrai contatta Aquafan per sapere a che evento avrai accesso", :contest_periodicity_id => contest_periodicity_3.id)
