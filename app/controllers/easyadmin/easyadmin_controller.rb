class Easyadmin::EasyadminController < ApplicationController
  ...
  def dashboard
    ...
    if User.any? 
      @from_time = params[:datepicker_from_date] == ("" || nil) ? User.order("created_at ASC").limit(1).first.created_at.strftime("%Y-%m-%d")
                  : Date.strptime(params[:datepicker_from_date], '%m/%d/%Y').to_date.strftime("%Y-%m-%d")
      @to_time = params[:datepicker_to_date] == ("" || nil) ? Time.now.to_date.strftime("%Y-%m-%d")
