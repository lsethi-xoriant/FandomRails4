module BrowseHelper
  ...
  def compute_cta_status_contents(contents, user)
    ...
    unless cta_ids.empty?
      cta_statuses = cta_to_reward_statuses_by_user(current_or_anonymous_user, CallToAction.includes(:interactions).where("id in (?)", cta_ids).references(:interactions).to_a, 'point')
    end
    contents.each do |content|
      if content.type == "cta"
